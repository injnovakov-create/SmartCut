...
with col1:
    ...
    # Инициализация на променливи, за да няма NameError
    appliances_type = "Без уреди"
    split_doors = False
    lower_door_h = 0
    lower_type = "Врата"
    vrati_broi = 1
    ch_heights = [] # Списък за височините на челата
    runner_len = 500
    ...
    if tip == "Дублираща страница долен":
        ...
        h_korpus = st.number_input("Височина на корпуса (H) мм", value=2040)
        ...
        if appliances_type != "Без уреди":
            ...
            split_doors = st.checkbox("Две врати (Долна + Горна)?", value=True)
        ...
        for i in range(num_ch):
            with cols_h[i]:
                val = st.number_input(f"Чело {i+1}", value=int(718/num_ch), key=f"ch_h_{i}")
                ch_heights.append(val)
        runner_len = st.number_input("Водач мм", value=500, step=50)
        d = st.number_input("Дълбочина (D) мм", value=520)
        h = 742 + kraka + 38
    else:
        w = st.number_input("Ширина (W) мм", value=150 if tip == "Шкаф Бутилки 15см" else 600)
        if "Горен" in tip:
            h = st.number_input("Височина (H) мм", value=720)
            d = st.number_input("Дълбочина (D) мм", value=300)
            vrati_broi = st.radio("Брой врати:", [1, 2], index=1 if w > 500 else 0, horizontal=True)
            vrati_orientacia = "Вертикални"
            if tip == "Горен Шкаф": vrati_orientacia = st.radio("Ориентация:", ["Вертикални", "Хоризонтални"], horizontal=True)
        else:
            d = st.number_input("Дълбочина (D) мм", value=520)
            h = 742 + kraka + 38 
            vrati_broi = st.radio("Брой врати:", [1, 2], index=1 if w > 500 else 0, horizontal=True)
    ...
    if st.button("➕ Добави към списъка"):
        ...
        # Мета за чертежи
        meta_dict = {"№": name, "Тип": tip, "W": w, "H": h, "D": d, "vr_cnt": vrati_broi}
        ...
        # ОБКОВ: Крака
        if any(x in tip for x in ["Долен", "Мивка", "Бутилки", "Фурна", "Чекмеджета", "Колона"]):
        ...
        # --- 2. ШКАФ 3 ЧЕКМЕДЖЕТА (ДИНИМИЧЕН) ---
            ...
            new_items.extend([
                ...
                add_item(name, tip, "Гръб (Фазер)", 1, h_korp_std - otstyp_f, w - otstyp_f, "Без", mat_fazer, "Няма")
            ])
            cw = w - (2*deb) - 49
            for idx, ch_h in enumerate(ch_heights):
                h_tsarga = ch_h - 60
                if h_tsarga < 70: h_tsarga = 70
        ...
        # --- 3. ШКАФ КОЛОНА ---
            ...
            w_izbr = int((w/vrati_broi) - fuga_obshto)
            new_items.extend([
                add_item(name, tip, "Дъно", 1, w, d, "1д", mat_korpus, val_fl_korpus),
                ...
                add_item(name, tip, "Гръб (Фазер)", 1, h_korpus - otstyp_f, w - otstyp_f, "Без", mat_fazer, "Няма")
            ])
            ...
            if lower_type == "Врата" or appliances_type == "Без уреди":
                l_h = lower_door_h if appliances_type != "Без уреди" or split_doors else h_korpus - fuga_obshto
                new_items.append(add_item(name, tip, "Врата долна/цяла", vrati_broi, l_h, w_izbr, "4 страни", mat_lice, val_fl_lice))
                new_hw.append({"№": name, "Артикул": "Панти", "Брой": calculate_hinges(l_h)*vrati_broi})
            
            h_up = h_korpus - lower_door_h - h_f - h_m - (fuga_obshto*2)
            if h_up > 150:
                new_items.append(add_item(name, tip, "Врата горна", vrati_broi, h_up, w_izbr, "4 страни", mat_lice, val_fl_lice))
                new_hw.append({"№": name, "Артикул": "Панти", "Брой": calculate_hinges(h_up)*vrati_broi})
