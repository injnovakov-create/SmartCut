...
def generate_labels_pdf(boards_per_mat):
    ...
    margin_x = int(7 * px_per_mm)  # (преместено с 3мм наляво)
    margin_y = int(9 * px_per_mm)  # (преместено с 3мм нагоре)
    
    # РАЗСТОЯНИЕ МЕЖДУ САМИТЕ ЕТИКЕТИ
    gap_x = int(6 * px_per_mm)
    # ПРОМЯНА: Увеличено разстояние по вертикала на 6.5мм (преди малко го направихме на 6)
