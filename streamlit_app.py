...
with col1:
    ...
    if cat_choice == "🍳 Кухненски Шкафове":
        icons = {"Стандартен Долен": "🗄️", "Горен Шкаф": "⬆️", "Шкаф Мивка": "🚰", "Шкаф 3 Чекмеджета": "🔢", "Шкаф Бутилки 15см": "🍾", "Шкаф за Фурна": "🍳", "Глух Ъгъл (Долен)": "📐", "Глух Ъгъл (Горен)": "📐"}
    else:
    ...
    # Инициализация
    has_appliances = False; appliances_type = "Без уреди"; split_doors = False; lower_door_h = 0; lower_type = "Врата"; vrati_broi = 1
    ...
    if tip == "Дублираща страница долен":
        ...
        h = custom_l = colA.number_input("Дължина (L) мм", value=600); d = custom_w = colB.number_input("Ширина (W) мм", value=300); w = deb; custom_count = colC.number_input("Брой", value=1, min_value=1)
        colD, colE = st.columns(2)
        custom_kant = colD.selectbox("Кант", ["Без", "1д", "2д", "1д+1к", "1д+2к", "2д+1к", "4 страни", "2д+2к"], index=6); custom_mat_type = colE.selectbox("Вид материал", ["Корпус", "Лице", "Чекмеджета", "Фазер"])
    elif tip == "Шкаф Колона":
        w = st.number_input("Ширина (W) мм", value=600); h_korpus = st.number_input("Височина без крака мм", value=2040); d = st.number_input("Дълбочина мм", value=550)
        appliances_type = st.radio("Вградени уреди:", ["Без уреди", "Само Фурна", "Само Микровълнова", "Фурна + Микровълнова"], horizontal=True)
        has_appliances = True if appliances_type != "Без уреди" else False
        if has_appliances:
            lower_door_h = st.number_input("Височина на долната част мм", value=718); lower_type = st.radio("Тип долу:", ["Врата", "2 Чекмеджета", "3 Чекмеджета"], horizontal=True)
        ...
        w = st.number_input("Ширина (W) на корпуса (мм)", value=(150 if tip == "Шкаф Бутилки 15см" else 600))
        if "Горен" in tip:
            h = st.number_input("Височина (H) в мм", value=720); d = st.number_input("Дълбочина (D) в мм", value=300)
            if tip == "Горен Шкаф": vrati_broi = st.radio("Брой врати:", [1, 2], index=1, horizontal=True); vrati_orientacia = st.radio("Ориентация:", ["Вертикални", "Хоризонтални (Клапващи)"], horizontal=True)
            elif tip == "Глух Ъгъл (Горен)": w_vrata_input = st.number_input("Ширина Врата (мм)", value=400); w_gluha_input = st.number_input("Ширина Глуха част (мм)", value=300)
        else:
            d = st.number_input("Дълбочина (D) мм", value=520)
            if tip == "Шкаф 3 Чекмеджета": runner_len = st.number_input("Дължина водач (мм)", value=500, step=50)
            elif tip == "Глух Ъгъл (Долен)": w_vrata_input = st.number_input("Ширина Врата (мм)", value=400); w_gluha_input = st.number_input("Ширина Глуха част (мм)", value=600)
            h = 742 + kraka + 38
            if tip in ["Стандартен Долен", "Шкаф Мивка"]: vrati_broi = st.radio("Брой врати:", [1, 2], index=(0 if w <= 500 else 1), horizontal=True)

    # --- ВИЗУАЛИЗАЦИЯ НА ЖИВО ---
    st.markdown("---")
    preview_img = draw_mini_preview({"Тип": tip, "W": w, "H": h, "D": d, "vr_cnt": vrati_broi}, kraka)
    ...
    if st.button("➕ Добави към списъка"):
       ...
       new_items = []
        ...
        # Записваме мета данните за PDF чертежите
        st.session_state.modules_meta.append({
            "№": name, "Тип": tip, "W": w, "H": h, "D": d, 
            "app_type": appliances_type, "ld_h": lower_door_h, "lower_type": lower_type,
            "vr_cnt": vrati_broi
        })

        # Добавяне на крака за всички долни модули и колони
        ...
        # --- ЛОГИКА ЗА КОЛОНА (ПОДОБРЕНА) ---
        if tip == "Шкаф Колона":
            w_izbrana = int((w/vrati_broi) - fuga_obshto)
            new_items.extend([
            ...
            # Изчисляване на ниши за уреди
            h_furn = 595 if "Фурна" in appliances_type else 0
            h_mw = 380 if "Микровълнова" in appliances_type else 0
            
            if h_furn > 0:
                new_items.append(add_item(name, tip, "Рафт тв. (под фурна)", 1, w-(2*deb), d, "1д", mat_korpus, val_fl_korpus))
                new_items.append(add_item(name, tip, "Рафт тв. (над фурна)", 1, w-(2*deb), d, "1д", mat_korpus, val_fl_korpus))
            if h_mw > 0:
                new_items.append(add_item(name, tip, "Рафт тв. (под МВ)", 1, w-(2*deb), d, "1д", mat_korpus, val_fl_korpus))
                new_items.append(add_item(name, tip, "Рафт тв. (над МВ)", 1, w-(2*deb), d, "1д", mat_korpus, val_fl_korpus))
            ...
            if lower_type == "Врата":
                ...
                cnt_ch = 2 if "2" in lower_type else 3
                ch_h = (lower_door_h / cnt_ch) - (fuga_obshto / 2)
                new_items.append(add_item(name, tip, "Чела долни", cnt_ch, ch_h, w - fuga_obshto, "4 страни", mat_lice, val_fl_lice))
                new_hw.append({"№": name, "Артикул": "Комплект водачи", "Брой": cnt_ch})

            # Горна врата (остатък от височината)
            h_upper_door = h_korpus - lower_door_h - h_furn - h_mw - (fuga_obshto * 2)
            if h_upper_door > 100:
                new_items.append(add_item(name, tip, "Врата горна", vrati_broi, h_upper_door, w_izbrana, "4 страни", mat_lice, val_fl_lice))
                new_hw.append({"№": name, "Артикул": "Панти покрит кант", "Брой": calculate_hinges(h_upper_door) * vrati_broi})

        # --- СТАНДАРТНИ МОДУЛИ (ОСТАВАТ СЪЩИТЕ) ---
        elif tip == "Шкаф 3 Чекмеджета":
            new_items.extend([
                add_item(name, tip, "Дъно", 1, w, d, "1д", mat_korpus, val_fl_korpus),
                add_item(name, tip, "Страница", 2, h_stranica, d, "1д", mat_korpus, val_fl_korpus),
                add_item(name, tip, "Чело горно", 1, 180-fuga_obshto, w-fuga_obshto, "4 страни", mat_lice, val_fl_lice),
                add_item(name, tip, "Чело средно", 1, 250-fuga_obshto, w-fuga_obshto, "4 страни", mat_lice, val_fl_lice),
                add_item(name, tip, "Чело долно", 1, 330-fuga_obshto, w-fuga_obshto, "4 страни", mat_lice, val_fl_lice)
            ])
            new_hw.extend([{"№": name, "Артикул": "Водачи", "Брой": 3}, {"№": name, "Артикул": "Дръжки", "Брой": 3}])
        
        # Тук си продължава твоята стандартна логика за "Стандартен долен", "Мивка" и т.н.
        # (Аз ги пропускам тук, за да не стане много дълъг текста, но ти си ги имаш в оригиналния код)

        st.session_state.order_list.extend(new_items)
        st.session_state.hardware_list.extend(new_hw)
        st.rerun()

# --- ТАБЛИЦА И PDF (ОСТАВАТ ПО ТВОЯ ДИЗАЙН) ---
with col2:
    ...
    if st.session_state.order_list:
        ...
        # Твоите функции за PDF (generate_technical_pdf и т.н.) следват най-отдолу...
...
def generate_technical_pdf(modules_meta, order_list, kraka_height):
    ...
    for mod in modules_meta:
        ...
        if "3 Чекмеджета" in tip and tip != "Шкаф Колона":
