...
def generate_labels_pdf(boards_per_mat):
    ...
    # ПРОМЯНА: Още 2мм наляво (от 7 намаляваме на 5)
    margin_x = int(5 * px_per_mm)  
    margin_y = int(9 * px_per_mm)  # (запазваме 3мм нагоре)
    
    # РАЗСТОЯНИЕ МЕЖДУ САМИТЕ ЕТИКЕТИ
    gap_x = int(6 * px_per_mm)
    gap_y = int(6.5 * px_per_mm)   # (запазваме увеличеното с 0.5мм)
    # -------------------------------------------------------------
