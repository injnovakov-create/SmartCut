...
with col1:
    ...
    if st.button("➕ ДОБАВИ"):
        ...
        # Логика за ПДЧ и детайлен обков
...
def generate_improved_pdf(meta, orders, k_h):
    ...
    for mod in meta:
        ...
        # ПОДОБРЕНИЕ 1: КРАЧЕТА (4 правоъгълника)
        if any(t in mod['Тип'] for t in ["Долен", "Мивка", "Чекмеджета", "Фурна"]):
            ...
            # Задни крака (леко изместени за 3D)
        ...
        # ПОДОБРЕНИЕ 2: ДВЕ ВРАТИ (Линия по средата)
        ...
        # ПОДОБРЕНИЕ 3: ЧЕЛА НА ЧЕКМЕДЖЕТА С РАЗМЕРИ
...
with c_p1:
    if st.button("📄 Генерирай PDF Чертежи"):
        ...
        st.download_button("📥 ИЗТЕГЛИ PDF", pdf_file, "SmartCut_Vitya_M.pdf")

# --- ФИНАНСИ (ЗАПАЗЕНИ И ПОДОБРЕНИ) ---
with c_p2:
    ...
    if st.session_state.order_list:
        ...
        # Груба сметка материали за екрана
        ...
        mat_cost = m2_total * 25 # Примерна цена
